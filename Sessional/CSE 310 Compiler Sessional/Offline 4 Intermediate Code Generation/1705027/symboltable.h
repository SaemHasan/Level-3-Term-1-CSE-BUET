...
class SymbolInfo{
    ...
    string tempsymbol="not set";//initialize to null later

    string funcRetType;
    bool   funcDefined=false;
    string variableDataType; // float, void , int
    string idType; // func, array, variable
...
public:
    ...
    SymbolInfo(const SymbolInfo &s){
      ...
      this->variableDataType = s.variableDataType;
      this->arraySize = s.arraySize;
      this->arrayIndex = s.arraySize;
      this->funcDefined = s.funcDefined;
      this->funcRetType = s.funcRetType;

      this->paramTypeList = s.paramTypeList;
      this->intArr = s.intArr;
      this->floatArr = s.floatArr;
      this->code = s.code;
      this->ids = s.ids;
      this->tempids = s.tempids;
      this->tempsymbol = s.tempsymbol;
      //this->next = s.next;
    }

    /*SymbolInfo operator = (const SymbolInfo &s){
      this->Name = s.Name;
      this->Type = s.Type;
      this->idType = s.idType;
      this->variableDataType = s.variableDataType;
      ...
      this->funcDefined = s.funcDefined;
      this->funcRetType = s.funcRetType;
      /*for(int i=0;s.parameterList.size();i++){
        this->parameterList.push_back(s.parameterList[i]);
      }*/
      /*this->paramTypeList = s.paramTypeList;
      this->intArr = s.intArr;
      this->floatArr = s.floatArr;
    ...
    void setTempSymbol(string t){tempsymbol = t;}

    void setFuncRetType(string type){funcRetType = type;}

    void setFuncDefined(bool defined){funcDefined = defined;}

    void setVariableDataType(string type){variableDataType = type;}

    void setIDType(string type){idType = type;}

    void setArraySize(unsigned int n){arraySize = n;}

    void setArrayIndex(unsigned int i){arrayIndex = i;}

    string getName(){return Name;}

    string getType(){return Type;}

    string getTempSymbol(){return tempsymbol;}

    string getFuncRetType(){return funcRetType;}

    bool getFuncDefined(){return funcDefined;}

    string getVariableDataType(){return variableDataType;}
...
class ScopeTable{
    ...
    public:
        ...
        bool Insert(SymbolInfo *s){
          ...
          // cout<<"printing details : \t "<<NewNode->getName()<<" "<<NewNode->getIDType()<<endl;
          int idx=hashFunc(NewNode->getName());
          ...
          // cout<<"inserted ScopeTable"<<endl;
          //cout<<NewNode->getName()<<"\tInserted in ScopeTable# " <<id<<" at position "<< idx<<", "<< cnt<<endl<<endl;
          return true;
        }
        ...
        SymbolInfo* Look_up(string key, string idtype){
            ...
            while(temp!=NULL){
                    if(temp->getName()==key && temp->getIDType()==idtype){
                        result = temp;
                        break;
                    }
                    cnt++;
...
class SymbolTable{
    ...
    public:
        ...
        void Enter_Scope(ofstream &out){
            ...
            out<<" New ScopeTable with id "<<newtableID<< " created"<<endl<<endl;
        }
        ...
        void Exit_Scope(ofstream &out){
            if(current_scopetable!=NULL){
                ScopeTable *temp=current_scopetable;
                out<<" ScopeTable with id "<<current_scopetable->id<<" removed"<<endl<<endl;
                current_scopetable=current_scopetable->parentScope;
                delete temp;
            }
            else{
        ...
        bool Insert(SymbolInfo *s){
          bool inserted = current_scopetable->Insert(s);
          return inserted;
        }
        ...
        SymbolInfo* Look_up(string key,string idtype){
            if(current_scopetable!=NULL){
                ...
                while(temp!=NULL){
                    //cout<<" key : "<<key<<endl;
                    result=temp->Look_up(key, idtype);
                    if(result!=NULL){
                         found=true;
                        break;
                    }
        ...
        SymbolInfo* Look_up_cur_scope(string key){
          SymbolInfo *result=NULL;
          result=current_scopetable->Look_up(key);
          return result;
        }
